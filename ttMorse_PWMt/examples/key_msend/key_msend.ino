...
//          Feb 16/15 - key_msend...initial startup string send to
//                      illustrate memory use of library functions
...
Bounce dot;
Bounce dash;
ttMorse_PWMt ttm( ledpin, tonepin, tfreq, cspwpm, str, risetime );
...
void loop( ) {
  ...
  // illustrate how library functions are only loaded if used
  if( first ) {
    ttm.msend( );
    if( !ttm.mbusy( ) ) first = false;
  }
